...
class Measurer_and_Filer:
  ...
  def png_this(self, fname):
    with open(fname, newline='') as data_file:
      ...
      y_thresh = [float(Measurer_and_Filer.thresh)]*len(y)
      #print (x)
      #print (y)
      print (y_thresh)
      ...
      ax = fig.subplots()
      ax.plot_date(x, y, linestyle='solid', color='#ffa500', 
        marker='o', label='Current draw')
      #ax.plot_date(x, new_y, label='time of use ___') #FOR SECOND LINE IN PLOT...doubt I'll use it
      fig.autofmt_xdate()
      ax.fill_between(x, y_thresh, y, where=(y > y_thresh), 
        interpolate=True, color='#ffa500', alpha=0.25, label='in use')
      ax.set_xlabel('time')
      ax.set_ylabel('Amps')
      ...
      if last_date != Measurer_and_Filer.date:
        ...
        if time.time() - last_time_txt > 1:	#txt refresh time
          ...
          if measurement > Measurer_and_Filer.thresh:
            area = ( (last_measurement + measurement)/2 )*1
            area_total += area
        ...
        elif time.time() - last_time_png > 5:	#plot refresh time
